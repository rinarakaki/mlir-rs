/*!
- lib <https://github.com/llvm/llvm-project/blob/main/mlir/lib/AsmParser/AsmParserImpl.h>
*/
...
impl AsmParser {
    pub fn new(name_loc: SMLoc, parser: &Parser) -> Self {
        Self { name_loc, parser, emitted_error: false }
    }
}
...
public:
  AsmParserImpl(SMLoc name_loc, Parser &parser)
      : name_loc(name_loc), parser(parser) {}
  ~AsmParserImpl() override = default;

  /// Return the location of the original name token.
  SMLoc getNameLoc() const override { return name_loc; }
  ...
  bool didEmitError() const { return emitted_error; }
  ...
  ParseResult parseAttribute(Attribute &result, r#type: Type) override {
  ...
  ParseResult parseCustomAttributeWithFallback(
      Attribute &result, r#type: Type,
      function_ref<ParseResult(Attribute &result, r#type: Type)> parseAttribute)
      override {
  ...
  ParseResult parseKeywordOrCompletion(StringRef *keyword) override {
    ...
    if (tok.isCodeCompletion() && tok.getSpelling().is_empty()) {
      *keyword = "";
      return success();
    }
